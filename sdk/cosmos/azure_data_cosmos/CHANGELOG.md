...
- Added the `pluggable_runtime` Cargo feature, which enables callers to replace the default reqwest-based HTTP transport and/or the default tokio async runtime used by [`CosmosClient`]. When enabled:
  - [`CosmosClientBuilder::with_driver_runtime_builder`] is no longer `#[doc(hidden)]` and is no longer gated on `__internal_in_memory_emulator`. Pass a pre-configured `CosmosDriverRuntimeBuilder` to swap in a custom `HttpClientFactory` (via `with_http_client_factory`), a custom `AsyncRuntime` (via `with_async_runtime`), or any other driver-level setting the SDK does not surface directly. The SDK's own settings (connection pool, the `azsdk-rust-cosmos/<version>` wrapping SDK identifier, the PPCB default, fault-injection rules, and throughput-control groups) are layered onto the supplied builder per the documented field-interaction rules.
  - A new `azure_data_cosmos::pluggable_runtime` module re-exports `CosmosDriverRuntime`, `CosmosDriverRuntimeBuilder`, the stable transport types (`HttpClientConfig`, `HttpClientFactory`, `HttpRequest`, `HttpResponse`, `HttpVersionPolicy`, `TransportClient`, `TransportError`), and the `AsyncRuntime` family (`AbortableTask`, `AsyncRuntime`, `SpawnedTask`, `TaskFuture`) so callers do not need to depend on `azure_data_cosmos_driver` directly.
  - Every public setter and re-export point carries an **Azure Support** doc-section pointing out that operations that run on a non-default plug point fall outside the configuration Microsoft validates and ships, so Microsoft cannot provide 24/7 Azure Support coverage for them; a repro on the default reqwest + tokio combination is required before a support investigation can proceed. Each operation's `DiagnosticsContext` records whether either plug point was in use via the new `custom_http_client` / `custom_async_runtime` flags on `azure_data_cosmos_driver::diagnostics::DiagnosticsContext`, so service-side readers can see at a glance which configuration produced a given trace.
### Breaking Changes
