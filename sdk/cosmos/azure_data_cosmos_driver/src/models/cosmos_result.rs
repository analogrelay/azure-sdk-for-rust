...
impl CosmosResult {
    ...
    pub fn into_parts(self) -> (Vec<u8>, CosmosHeaders, Arc<DiagnosticsContext>) {
        (self.body, self.headers, self.diagnostics)
    }

    /// Returns a reference to the extracted headers.
    pub fn headers(&self) -> &CosmosHeaders {
        &self.headers
    }
...
impl CosmosHeaders {
    ...
    /// Sets the ETag.
    pub(crate) fn with_etag(mut self, etag: String) -> Self {
        self.etag = Some(etag);
        self
    }

    /// Sets the continuation token.
    pub(crate) fn with_continuation(mut self, continuation: String) -> Self {
        self.continuation = Some(continuation);
        self
    }

    /// Sets the item count.
    pub(crate) fn with_item_count(mut self, count: u32) -> Self {
