...
impl std::fmt::Display for RequestTarget {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            RequestTarget::NonPartitioned => write!(f, "non-partitioned"),
            ...
            RequestTarget::EffectivePartitionKeyRange {
                ...
                if range.is_none() {
                    write!(f, "pkrange({partition_key_range_id})")
                } else {
                    write!(
                        f,
                        "pkrange({partition_key_range_id}, [{},{}))",
                        range.as_ref().unwrap().min_inclusive(),
                        range.as_ref().unwrap().max_exclusive()
                    )
                }
            }
        }
...
impl PipelineNode for Request {
    #[tracing::instrument(name = "request", skip_all, fields(
        target = %self.target,
        state = ?self.state,
    ))]
    async fn next_page(
        ...
        tracing::debug!("executing request node");
