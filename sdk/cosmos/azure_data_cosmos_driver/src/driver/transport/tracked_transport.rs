...
impl EventEmitter {
    ...
    pub fn emit_type(&self, event_type: RequestEventType) {
        self.emit(RequestEvent::new(event_type));
    }

    /// Emits an event with details.
    pub fn emit_with_details(&self, event_type: RequestEventType, details: impl Into<String>) {
...
pub struct TrackedRequestState {
...
impl TrackedRequestState {
    ...
    pub fn collect(receiver: EventReceiver) -> Self {
